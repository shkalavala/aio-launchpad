...
import { Cloud, Sparkles, Layers, Wand2 } from "lucide-react";
  ...
  return (
    <header className="flex h-12 items-center gap-6 border-b border-border bg-surface px-4">
      ...
      <div className="ml-auto flex items-center gap-3">
        <Link
          href="/v2"
          title="Open the v2 fresh-look preview — a git-native fleet management surface. Your classic experience stays untouched."
          className="inline-flex h-7 items-center gap-1.5 rounded-full border border-accent/40 bg-accent-subtle px-2.5 text-[11px] font-medium text-accent transition-colors hover:border-accent"
        >
          <Wand2 className="h-3 w-3" />
          Try v2
          <span className="ml-0.5 rounded-full bg-accent px-1.5 py-px text-[10px] font-semibold uppercase tracking-wide text-white">
            preview
          </span>
        </Link>
        <button
